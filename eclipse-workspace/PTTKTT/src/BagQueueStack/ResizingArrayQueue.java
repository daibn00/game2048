...
public class ResizingArrayQueue<Item> implements Iterable<Item> {
	...
	public static void main(String[] args) throws IOException {
		FileReader fr = new FileReader(new File("tobe.txt"));
		BufferedReader br = new BufferedReader(fr);

		ResizingArrayQueue<String> raQueue = new ResizingArrayQueue<String>();
		String line = br.readLine();
		String str[] = line.split("\\s");
		for (int i = 0 ; i < str.length; i++) {
			if (!str[i].equals("-")) {
				raQueue.enqueue(str[i]);
			}
			else if (!raQueue.isEmpty()) System.out.println(raQueue.dequeue() + " ");
		}
		System.out.println("(" + raQueue.size() + " left on queue)");
